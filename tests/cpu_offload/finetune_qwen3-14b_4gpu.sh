...
# Per DeepSpeed: train_gbs = mbs * gas * dp_size
...
# same (mode, gbs, mbs) do not silently overwrite earlier results.
...
echo "Qwen3-14B | mode=${MODE} gbs=${GBS} mbs=${MBS} gas=${GAS} (derived) cpu_ratio=${CPU_RATIO} profile=${PROFILE}"
